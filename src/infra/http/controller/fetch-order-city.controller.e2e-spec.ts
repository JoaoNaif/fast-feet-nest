...
describe('Fetch Order City (E2E)', () => {
  ...
  test('[GET] /deliveryman/city/:deliverymanId', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get(`/deliveryman/city/${deliveryman.id.toString()}?query=Bauru`)
    ...
    expect(response.body).toEqual({
      deliveryman: expect.objectContaining({
        recipientNearby: expect.arrayContaining([
          expect.objectContaining({ city: 'Bauru' }),
        ]),
      }),
    })
  })
