...
describe('Fetch Nearby Delivery', () => {
  ...
  it('must be able to search for all deliveries within 10km', async () => {
    ...
    await inMemoryRecipentRepository.create(recipient3)
    ...
    if (result.isRight()) {
      ...
      expect(deliveries.recipientNearby[0].city).toBe('São Paulo')
    }
  })

  it('should be able to fetch paginated search cities', async () => {
    const deliveryman = makeDeliveryman(
      {
        role: 'ADMIN',
        latitude: -23.5591942,
